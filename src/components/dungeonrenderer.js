// This code was taken from https://dungeoncrawlers.org/tools/dungeonrenderer/
// This is not exact copy because it has some adjustments to make it work without fetching files internally

let _dungeonRenderer

export default function DungeonRenderer(
  container,
  atlasImagePath,
  jsonData,
  mapData,
  ...
  this.dungeon = jsonData
  this.atlasImage = null
  this.atlasSrc = atlasImagePath
...
DungeonRenderer.prototype.loadAtlasImage = function () {
  ...
  caller.atlasImage.src = caller.atlasSrc
  // Используем переданный объект Image
  caller.atlasImage.onload = function () {
    caller.generateImages()
  }

  // Убедимся, что изображение загружено
  if (caller.atlasImage.complete) {
    caller.atlasImage.onload()
  }
}
...
DungeonRenderer.prototype.generateImages = function () {
  ...
  this.dungeon.layers.forEach((layer, layerIndex) => {
    ...
    layer.tiles.forEach((entry, i) => {
      ...
      var imageData = atlasCtx.getImageData(
        entry.coords.x,
        entry.coords.y,
        entry.coords.w,
        entry.coords.h
...
DungeonRenderer.prototype.render = function () {
  ...
  if (this.floorLayers.length == 1) {
    var result = null
    if ((result = this.getImage(this.floorLayers[0].index, 'floor', -1, -1))) {
      this.ctx.drawImage(result.image, result.screen.x, result.screen.y)
    }
  }
  ...
  if (this.ceilingLayers.length == 1) {
    var result = null
    if ((result = this.getImage(this.ceilingLayers[0].index, 'ceiling', -1, -1))) {
      this.ctx.drawImage(result.image, result.screen.x, result.screen.y)
    }
  }
